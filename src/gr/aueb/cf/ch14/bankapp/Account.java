...
public class Account {
    ...
    public void withdraw(double amount, String ssn) throws NegativeAmountException, InsufficientBalanceException, SsnNotValidException {
        try {
            ...
            if (amount > balance) throw new InsufficientBalanceException("The amount exceeds the balance");
            if (!isSsnValid(ssn)) throw new SsnNotValidException("SSN does not match");
            balance -= amount;
        } catch (NegativeAmountException | InsufficientBalanceException | SsnNotValidException e) {
            System.err.printf(LocalDateTime.now().toString() + "Withdrawal failed. \n%s\n", e.getMessage());
            throw e;
        }
    }
